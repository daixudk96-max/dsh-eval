...
| P2(治理增强 ✅ 2026-08-23) | 治理: 逆编辑回滚(registry.rollbackContent + lib/rollback.js, 无 LLM)+ partial 冲突检测(中间变更交集→报错, --force 覆盖)+ 写前威胁扫描(lib/threat.js 三类, 挂 createCandidate/promote)+ failures 失败类聚合(dsh-evolve failures)+ compare --delta decisionReport(任务 feat-08-23-p2-governance-enhance) | P1-E | rollback-content 7 测 + controller-rollback 4 测 + threat 7 测 + failures 4 测全绿; dsh-eval compare/command delta 全绿 + mirror tsc 0 错; 真实 failures 聚合 run.json + 真实 compare --delta fail→pass; 全量回归绿(evolution-controller 17 文件 / preset-registry 3 文件 / dsh-eval 200 测) |
| P3(内容/记忆 ✅ 2026-08-23) | 内容/记忆域(任务 feat-08-23-p3-content-memory): skill 提炼(lib/skill-extract.js 吸收 dsh-skill-evolve, 参数泛化+步骤去重)+ harness state 四类版本化(lib/state-store.js 映射 Registry API, 按 id 回滚)+ skill 生命周期(lib/skill-lifecycle.js active→stale→archived + pinned/restore/consolidate)+ 热挂载验证(隔离 DSH_HOME + 真实 FileSystemSkillProvider → FEASIBLE, 零组合改动)+ 记忆文件(lib/memory.js 预算+dedup)+ ranked injection(lib/injection.js ≤6/kind×180 字符, 空 store 零 token) | P2(治理增强) | 新 5 模块 55 测全绿(skill-extract 13 / state-store 11 / skill-lifecycle 14 / memory 9 / injection 8); 真实 session(30,518 events)→SKILL.md 提炼成功; hot-mount 7 断言含热新增 FEASIBLE(见 research/hot-mount-verification.md); 全量回归 evolution-controller 22 文件 168 测 + preset-registry 3 文件 21 测; registry.js 仅增 _safeFileId(%→%25, :→%3A)向后兼容 |
